...
function renderExpenses() {
  //if there is something in localStorage pass it into allExpenses array
  ...
  if (allExpenses.length > 0) {
    ...
    for (i = 0; i < allExpenses.length; i++) {
      //create LI element with span element as a name and button for erasing it
      ...
      //set id to the button with the same id as a li has
      smazat.setAttribute("id", allExpenses[i].id);
      //onclick function for an erase btn, which takes the id of the button and removes it
    ...
    //if there is no expense, fill the total amount with nothing
...
function removeExpense(expenseId) {
  //filter function for returning the same array, but without the one with the ID
